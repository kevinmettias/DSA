...
// regardless of blacklist size. Only WhitelistSize (a small constant) numbers are ever
// left un-blacklisted while RangeSize grows, so a uniform draw's success probability is
// ~WhitelistSize/RangeSize - rejection sampling's expected retries per pick grow
// linearly with RangeSize, while the remap solution's Pick() stays O(1) after its
// one-time O(B) constructor-equivalent setup. [GlobalSetup] only builds the blacklist

...
// Best Time to Buy and Sell Stock with Transaction Fee (LC 714): plain un-memoized
// recursion over (day, holding) - exponential, since the same state recurs along
// many different buy/sell/rest paths - vs. this repo's own Memoizer<TState,TResult>
// caching that exact pair. Length is kept modest (<=28) specifically because the
// un-memoized baseline's blowup is real, the same reasoning
...
public class BestTimeToBuyAndSellStockWithTransactionFeeBenchmarks
{
    ...
    private int ProfitFrom(int day, bool holding)
    {
        ...
        if (holding)
        {
            var sell = _prices[day] - Fee + ProfitFrom(day + 1, false);
    ...
    public int MemoizedRecursion()
    {
        ...
        int ProfitFromMemoized((int Day, bool Holding) state, Func<(int Day, bool Holding), int> profit)
        {
            ...
            if (holding)
            {
                var sell = _prices[day] - Fee + profit((day + 1, false));

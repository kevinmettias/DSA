...
// arrangements it must walk before the k-th one, not the alphabet's full
// arrangement count, so this is the size that keeps it a fair, terminating
// comparison rather than a demonstration that unbounded k is intractable for it. A
// small 4-letter alphabet keeps the half's own arrangement count comfortably above
...
public class SmallestPalindromicRearrangementIIBenchmarks
{
    ...
    public string BacktrackingRank() => SmallestPalindromicRearrangementIISolution.RearrangeByBacktrackingRank(_s, Rank);

    [Benchmark]
    public string CountingGreedy() => SmallestPalindromicRearrangementIISolution.RearrangeByCountingGreedy(_s, Rank);
}

...
// SoupServingsTests proves correct. The two N values deliberately straddle the
...
// un-memoized game-tree baselines, here made sharper by soup's branching factor of
// 4 (vs. those two problems' factor of 2-3). Both sizes sit below the solution's
// large-n short circuit, so each arm runs its full search.
//
// There is no [GlobalSetup] left to charge: the only preparation the old harness
// did was quantizing N to servings, one integer division that is part of the
// LeetCode-shaped call itself and identical for both arms.
[MemoryDiagnoser]
public class SoupServingsBenchmarks
{
    ...
    [Benchmark]
    public double MemoizedRecursion() => SoupServingsSolution.ProbabilityByMemoizedRecursion(N);
}

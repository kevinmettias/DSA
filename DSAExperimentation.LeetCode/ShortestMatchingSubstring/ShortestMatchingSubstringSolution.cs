...
{
    public static int ShortestLengthByBruteForceIndexOf(SourceText text, WildcardPattern pattern) =>
        ShortestLengthByBruteForceIndexOf(text, MatchPattern.Parse(pattern.Text));
    ...
    public static int ShortestLengthByBruteForceIndexOf(SourceText text, MatchPattern pattern)
    {
        var best = LeetCodeAnswer.None;
        var prefixStart = IndexOfFrom(text, new LiteralPart(pattern.Prefix), 0);

        while (prefixStart >= 0)
        {
            var middleStart = IndexOfFrom(text, new LiteralPart(pattern.Middle), prefixStart + pattern.Prefix.Length);

            if (middleStart >= 0)
            {
                var suffixStart = IndexOfFrom(text, new LiteralPart(pattern.Suffix), middleStart + pattern.Middle.Length);
            ...
            prefixStart = IndexOfFrom(text, new LiteralPart(pattern.Prefix), prefixStart + 1);
        }
    ...
    public static int ShortestLengthByKmpBinarySearch(SourceText text, WildcardPattern pattern)
    {
        var occurrences = PatternOccurrences.Build(text.Text, MatchPattern.Parse(pattern.Text));
    ...
    // SourceText is the text a match is looked for in and WildcardPattern the pattern
    // it is looked for by, and each of the pattern's three literal parts is what a
    // substring search is pointed at - but as three bare `string` positions nothing
    // stopped a caller from handing the text over as the pattern, or the pattern over
    // as the fragment to find, and the search would have answered the wrong question
    // with no complaint from the compiler.
    internal readonly record struct SourceText(string Text);

    // The pattern as LeetCode hands it over: the whole pattern with both '*' wildcards
    // still in place, before MatchPattern.Parse splits it on them. Distinct from
    // MatchPattern because it is not yet three parts, and from SourceText because it is
    // what the source is searched for rather than what is searched.
    internal readonly record struct WildcardPattern(string Text);

    // One of the three literal fragments MatchPattern.Parse splits the wildcard pattern
    // into, and the unit IndexOfFrom seeks inside a SourceText. It is the sought side of
    // that search: the two swapped asks whether the text occurs inside the fragment,
    // which is not the question either caller means to ask.
    internal readonly record struct LiteralPart(string Text);

    // The wildcard pattern split on its two '*' characters into the three literal parts
    // either side must match - a witness meaningful only to this problem, so it lives in
    // this folder rather than Domain/ (ARCHITECTURE.md §17.3). Nested because it is this
    ...
    {
        public static MatchPattern Parse(string pattern)
        {
    ...
    // pattern's three literal parts within the text, computed once so a benchmark
    // measures the greedy binary-search combination alone - mirroring how
    // OpenTheLockBenchmarks hoists LockGraph.Build out of the search it measures.
    ...
    {
        public static PatternOccurrences Build(string text, MatchPattern pattern) =>
            new(
                pattern,
                PrefixFunctionSearch.FindAll(text, pattern.Prefix).ToArray(),
                PrefixFunctionSearch.FindAll(text, pattern.Middle).ToArray(),
                PrefixFunctionSearch.FindAll(text, pattern.Suffix).ToArray());
    }
}

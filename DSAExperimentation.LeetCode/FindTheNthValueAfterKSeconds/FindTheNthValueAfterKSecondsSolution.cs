...
// LeetCode 3179. Find the N-th Value After K Seconds: an array of n ones is
// updated k times, each second replacing every a[i] (i >= 1, left to right, so
// each update sees the already-updated a[i-1]) with a[i-1] + a[i]; report
// a[n-1] mod 1e9+7.
//
...
{
    // The textbook answer: run the k seconds literally, one running-prefix-sum
    ...
    public static int ValueAfterKSecondsByBruteForce(int n, int k)
    {
        var values = new long[n];
        Array.Fill(values, 1L);

        for (var second = 0; second < k; second++)
        {
            for (var i = 1; i < n; i++)
            {
    ...
    // Composed: k rounds of prefix-summing an all-ones array is exactly Pascal's
    // triangle by another name, so a[n-1] after k seconds is the closed-form
    // binomial coefficient C(n - 1 + k, k). Computed as a factorial ratio using
    // Domain.Modular's Inverse (Fermat's-little-theorem modular inverse) for the
    ...
    // shares. O(n + k).
    public static int ValueAfterKSecondsByModularBinomial(int n, int k)
    {
        var upper = n - 1 + k;
        ...
        var denominator = factorial[k] * factorial[n - 1] % ModularArithmetic.Modulo;

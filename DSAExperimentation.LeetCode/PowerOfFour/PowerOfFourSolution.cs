...
// LeetCode 342. Power of Four: is `value` exactly 4^k for some non-negative integer k?
//
...
internal static class PowerOfFourSolution
{
    ...
    public static bool IsPowerOfFourByDivisionLoop(int value)
    {
        if (value <= 0)
        {
            return false;
    ...
    // single overload.
    public static bool IsPowerOfFourByBinarySearch(int value) =>
        BinarySearch.Find<int, PowersOfFourSequence>(new PowersOfFourSequence(), value) is not null;
}

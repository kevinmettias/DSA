...
// n pickups and n deliveries in which every P_i precedes its own D_i, modulo 1e9+7.
//
...
internal static class CountAllValidPickupAndDeliveryOptionsSolution
{
    ...
    // Textbook baseline: walk i = 1..n carrying one running product, reducing mod
    // 1e9+7 after each multiplication so nothing overflows. BCL arithmetic only.
    public static long CountOrdersByTabulation(int n)
    {
        var ways = 1L;

...
// smallest |nums[i] - nums[j]| over pairs of indices at least x apart.
//
...
// 0..j-x, and that set grows by exactly one element per step - index j - x, which
// only just became eligible - so the structure is built incrementally by the same
// loop that queries it, never rebuilt.
internal static class MinimumAbsoluteDifferenceBetweenElementsWithConstraintSolution
{
    // The textbook answer: for every i, walk every partner at or beyond i + x and
    // keep the smallest gap seen. Plain BCL indexing and Math.Abs/Math.Min, nothing
    // from this repo - it is the O(n^2) arm the sliding-window strategy below has to
    // justify itself against, and stating it here is what finally gets it asserted.
    public static int MinAbsoluteDifferenceByBruteForcePairScan(int[] nums, int x)
    {
        var minDifference = int.MaxValue;

        for (var i = 0; i < nums.Length; i++)
        {
            for (var j = i + x; j < nums.Length; j++)
            {
    ...
    public static int MinAbsoluteDifferenceByBstSlidingWindow(int[] nums, int x)
    {
        ...
        {
            // Index j - x is the one partner that becomes eligible at step j; every
            // earlier one is already in the tree.
            if (j >= x)
            {
                eligiblePartners.Insert(nums[j - x]);
            }

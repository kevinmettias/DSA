...
// lands her on the target stair, where she may go down one stair (never twice in a
// row, never from stair 0) or up stair + 2^jump (then jump increments). Once the
// current stair exceeds targetStair + 1, no further sequence of moves can ever land
// back on targetStair - a single down nets -1 but must be followed by an up that
// adds at least 2^0 = 1, so the position never decreases below where it already
// overshot - which is what bounds both strategies below to O(log k) recursion depth
// despite targetStair running up to 1e9.
internal static class FindNumberOfWaysToReachTheKthStairSolution
{
    ...
    public static int WaysByBruteRecursion(int targetStair) =>
        CountWays(1, 0, DownStep.Available, targetStair);
    ...
    /// 1 when this is stair <paramref name="targetStair"/>, plus the count after the up move, plus
    /// the count after the single down move while it is still available - and 0 once the
    /// stair has overshot the target stair + 1, which is what bounds the depth.
    /// </summary>
    private sealed class StairWalkTo(int targetStair) : IRecurrence<StairState, int>
    {
        /// <inheritdoc/>
        public int Replay(StairState state, IRecurrence<StairState, int> rest)
        {
            if (state.Stair > targetStair + 1)
            {
                return 0;
            }

            var total = state.Stair == targetStair ? 1 : 0;
    ...
    private static int CountWays(long stair, int jump, DownStep downStep, int targetStair)
    {
        if (stair > targetStair + 1)
        {
            return 0;
        }

        var ways = stair == targetStair ? 1 : 0;
        ways += CountWays(stair + (1L << jump), jump + 1, DownStep.Available, targetStair);

        if (downStep == DownStep.Available && stair > 0)
        {
            ways += CountWays(stair - 1, jump, DownStep.Spent, targetStair);
        }

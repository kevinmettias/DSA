...
// LeetCode 397. Integer Replacement: fewest operations to reduce startValue to 1,
// where an even startValue may only be halved and an odd one may only be
// incremented or decremented - replace(n) = 0 when n == 1, 1 + replace(n/2) when n
// is even, and 1 + min(replace(n-1), replace(n+1)) when n is odd.
//
...
// LeetCode's own n is an int: startValue can be int.MaxValue (2^31 - 1), and the
// n+1 branch on the odd case would otherwise silently overflow Int32.
internal static class IntegerReplacementSolution
{
    ...
    public static int MinStepsByUnmemoizedRecursion(int startValue) => Replace(startValue);
    ...
    public static int MinStepsByMemoizedRecurrence(int startValue)
        => Memoizer.Memoize<long, int>(startValue, new StepsToReduceToOne());

    private static int Replace(long value)
        => value switch
        {
            1 => 0,

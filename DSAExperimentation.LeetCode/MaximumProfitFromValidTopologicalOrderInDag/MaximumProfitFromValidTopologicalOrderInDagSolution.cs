...
// sum(score[node] * position) is maximised. n <= 22, so "which nodes have
// already been placed" fits in one int bitmask - the same Held-Karp-shaped
...
internal static class MaximumProfitFromValidTopologicalOrderInDagSolution
{
    ...
    public static long MaxProfitByBacktracking(int n, int[][] edges, int[] score)
    {
        var predecessorMasks = BuildPredecessorMasks(n, edges);
    ...
    private static long SearchBestProfit(int mask, int placed, int[] predecessorMasks, int[] score)
    {
        ...
        for (var v = 0; v < score.Length; v++)
        {
            ...
            var profit = ((long)score[v] * (placed + 1))
                + SearchBestProfit(mask | (1 << v), placed + 1, predecessorMasks, score);
    ...
    private static int[] BuildPredecessorMasks(int n, int[][] edges)
    {
        var predecessorMasks = new int[n];

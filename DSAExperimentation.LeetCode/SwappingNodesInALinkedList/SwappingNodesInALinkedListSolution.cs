...
// head. LeetCode guarantees 1 <= k <= n, so both positions always exist.
//
...
// the values into a BCL list turns it into index n - k at the cost of O(n) extra
// space; a fast/slow pair of node references finds it with no extra space at all,
// because a runner started k - 1 nodes ahead runs out exactly when the trailing
// reference is standing on it.
internal static class SwappingNodesInALinkedListSolution
{
    ...
    // from the end is just index n - k, swap the two entries there and build a
    // fresh list from the result. Deliberately BCL-only beyond the input/output
    // list shape itself - it is the arm the two-pointer walk below has to justify
    // itself against.
    public static SinglyLinkedListNode<int>? SwapNodesByArrayMaterialize(SinglyLinkedListNode<int>? head, int k)
    {
    ...
    public static SinglyLinkedListNode<int>? SwapNodesByTwoPointerWalk(SinglyLinkedListNode<int>? head, int k)
    {
        var front = head;

...
// whose elements sum to a multiple of divisor?
internal static class ContinuousSubarraySumSolution
{
    ...
    public static bool HasSubarraySumMultipleOfKByBruteForce(int[] nums, int divisor)
    {
        for (var start = 0; start < nums.Length; start++)
        {
            ...
            {
                sum += nums[end];
                if (end - start >= MinimumSubarrayLength - 1 && sum % divisor == 0)
                {
                    return true;
                }
            }
    ...
    // remainder means the subarray between them sums to a multiple of divisor; seeding the
    // map with {0: -1} lets a subarray starting at index 0 register the same way as
    // any other.
    public static bool HasSubarraySumMultipleOfKByHashMapPrefixRemainder(int[] nums, int divisor)
    {
        var firstIndexByRemainder = new HashMap<int, int>();
        firstIndexByRemainder.Set(0, -1);

        return HasRepeatedRemainder(nums, divisor, firstIndexByRemainder);
    }
    ...
    private static bool HasRepeatedRemainder(int[] nums, int divisor, HashMap<int, int> firstIndexByRemainder)
    {
        var prefixSum = 0;
        for (var i = 0; i < nums.Length; i++)
        {
            prefixSum += nums[i];
            var remainder = prefixSum % divisor;

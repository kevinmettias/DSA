...
// IRandomAccessSequence<int> witness over partition index in [0, nums1.Length]:
// Get(index) is 0 while nums1[index-1] <= nums2[half-index] (this partition still
// leaves every element to nums1's left no bigger than nums2's right-hand neighbor)
// and flips to 1 once it doesn't - a single monotonic step, never re-examined once
// it flips, so BinarySearch.LowerBound's existing loop finds the boundary in O(log m)
// without ever touching nums2's own length. This answers LC 4's partition search and

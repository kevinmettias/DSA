...
internal static class MaximumTotalBeautyOfTheGardensSolution
{
        ...
        int[] flowers, long newFlowers, int target, int full, int partial)
    {
        ...
        var (context, prefixSum, suffixCost) =
            PrepareSplitState(sorted, newFlowers, target, new BeautyWeights(full, partial));

        return RunSplitSearch(
            context,
            suffixCost,
            (i, remaining) => LinearMaxAchievableHeight(sorted, new PrefixContext(prefixSum, i), remaining, target - 1));
    }
        ...
        int[] flowers, long newFlowers, int target, int full, int partial)
    {
        ...
        return RunSplitSearch(
            context,
            suffixCost,
            (i, remaining) => BinarySearchMaxAchievableHeight(sequence, new PrefixContext(prefixSum, i), remaining, target - 1));
    }

    // LeetCode reports the two beauty rates as separate ints; they only ever travel
    // together, and only the split scoring below reads them.
    ...
    private static long RunSplitSearch(SplitContext context, long[] suffixCost, Func<int, long, long> heightAt)
    {
        var best = 0L;
        ...
        for (var i = 0; i <= context.N; i++)
        {
            ...
            {
                best = Math.Max(best, BeautyForSplit(context, i, completionCost, heightAt));
            }
        }
    ...
    private static long BeautyForSplit(SplitContext context, int i, long completionCost, Func<int, long, long> heightAt)
    {
        var height = 0L;

        if (i > 0 && context.AnyGardenIncomplete)
        {
            height = heightAt(i, context.Budget - completionCost);
        }

...
// applies). `targetPosition` can reach 10^16, so the doubling word itself may
// never actually be built - only its length is tracked round by round, and the
// target position is traced backward through the operations to the single seed
...
internal static class FindTheKthCharacterInStringGameIISolution
{
    ...
    public static char KthCharacterByBruteForceSimulation(long targetPosition, int[] operations)
    {
        ...
        return word[(int)(targetPosition - 1)];
    }

    private static char NextChar(char character) => character == 'z' ? 'a' : ShiftedChar(character);

    // One letter on in the alphabet.
    private static char ShiftedChar(char character) => (char)(character + 1);
    ...
    // after each operation - frozen once it exceeds targetPosition, since the
    // position (which only ever shrinks from targetPosition) can then never fall
    // in the untouched first half again, so the exact magnitude beyond that point
    // is irrelevant and freezing it is what keeps the running total from
    // overflowing a long across up to 100 doublings. The position is then traced
    // back through the operations those lengths came from.
    public static char KthCharacterByBackwardTrace(long targetPosition, int[] operations)
    {
        ...
        {
            var isRoundFrozen = lengths[i] > targetPosition;
    ...
    // Walks the operations backward from the final length: at each step the position
    ...
    private static int TraceShiftBackward(long targetPosition, int[] operations, long[] lengths)
    {

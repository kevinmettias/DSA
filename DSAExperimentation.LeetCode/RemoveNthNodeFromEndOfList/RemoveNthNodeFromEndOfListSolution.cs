...
// remove the node that sits n positions from the end (1-indexed) and return the new
// head.
//
// Both strategies land on the same target - index Length - n, 0-indexed from the
// head - but reach it differently. The baseline materializes the list into an
// array and rebuilds one around the gap; the two-runner walk finds the same spot
// in a single pass with no extra storage, by running a fast pointer n nodes ahead
// of a slow one so the slow pointer stops just before the target the instant the
// fast pointer runs off the end.
internal static class RemoveNthNodeFromEndOfListSolution
{
    ...
    public static SinglyLinkedListNode<int>? RemoveByArrayRebuild(SinglyLinkedListNode<int>? head, int n)
    {
        var values = ToArray(head);
        var index = values.Length - n;
    ...
    public static SinglyLinkedListNode<int>? RemoveByTwoRunner(SinglyLinkedListNode<int>? head, int n)
    {

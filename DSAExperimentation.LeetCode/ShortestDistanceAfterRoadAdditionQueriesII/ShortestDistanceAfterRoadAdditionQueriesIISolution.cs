...
// LeetCode 3244. Shortest Distance After Road Addition Queries II: n cities sit on a
// chain 0 -> 1 -> ... -> n-1; each query adds a one-way shortcut u -> v (v > u + 1) and
// asks for the shortest 0 -> n-1 path length afterward. LC guarantees queries never
...
// guarantee rules out. So the answer after each query is just (n - 1) minus however
// many interior cities have been bypassed so far - and "which cities have been
...
internal static class ShortestDistanceAfterRoadAdditionQueriesIISolution
{
    ...
    public static int[] ShortestDistancesByAdjacencyBfs(int n, int[][] queries)
    {
        ...
        for (var q = 0; q < queries.Length; q++)
        {
            ...
            adjacency[u].Add(v);
            answers[q] = ShortestPathLength(adjacency, n);
        }
    ...
    // cities - and therefore the answer, (n - 1) minus that total - only ever needs
    // this query's own new range, never the whole history.
    public static int[] ShortestDistancesByIntervalSet(int n, int[][] queries)
    {
        ...
        for (var q = 0; q < queries.Length; q++)
        {
            ...
            answers[q] = n - 1 - CountBypassed(bypassed);
        }

...
// LeetCode 347. Top K Frequent Elements: the k values that occur most often in
// nums, in any order.
//
...
// counts with this repo's own HashMap<int, int>, then keeps only the k most
// frequent in a size-k min-heap ordered by ByPriorityOrder<TNode, TWeight> (the
...
// the heap grows past k, the same size-k-heap shape KthLargestElementSolution
// uses for LeetCode 215.
internal static class TopKFrequentElementsSolution
{
    // The textbook approach: count with a BCL Dictionary, then sort every
    // distinct value by frequency and take the top k. Deliberately written
    // without this repo's primitives - the arm the size-k heap strategy below
    // has to justify itself against.
    public static int[] FindTopKFrequentByFullSort(int[] nums, int k)
    {
    ...
    public static int[] FindTopKFrequentBySizeKMinHeap(int[] nums, int k)
    {
        var counts = CountFrequencies(nums);
        var heap = BuildSizeKMinHeap(counts, k);

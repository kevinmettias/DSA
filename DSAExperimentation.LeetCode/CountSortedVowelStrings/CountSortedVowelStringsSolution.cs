...
// LeetCode 1641. Count Sorted Vowel Strings: how many strings of the given length
// consist only of vowels and are sorted lexicographically. A sorted string never
// steps back to an earlier vowel, so the answer is the number of non-decreasing
// sequences of that length over the five vowels - a choice at vowel v may only be
// followed by choices >= v, which is the one rule both strategies below encode.
//
...
internal static class CountSortedVowelStringsSolution
{
    ...
    public static int CountVowelStringsByBacktrackingEnumeration(int length)
    {
        var strings = new List<string>();

        Build(new char[length], position: 0, start: 0, strings);
    ...
    public static int CountVowelStringsByMemoizedRecurrence(int length) =>
        Memoizer.Memoize<(int Remaining, int Start), int>((length, 0), new ChoicesFromAllowedVowel());

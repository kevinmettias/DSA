...
public sealed partial class LeastOperatorsToExpressNumberTests
{
    public static TheoryData<int, int, int> Examples =>
        new()
        {
            // LC example 1: baseValue = 3, target = 19 -> "3*3 + 3*3 + 3/3", 5 operators.
            { 3, 19, 5 },

            // LC example 2: baseValue = 5, target = 501 -> 8 operators.
            { 5, 501, 8 },

            // LC example 3: baseValue = 100, target = 100000000 -> "100*100*100*100", 3.
            { 100, 100_000_000, 3 },

            // The target is the base itself: written as "x", with no operators.
            { 5, 5, 0 },

            // baseValue = 2 is the one base where quotient+1 can land back on the same
            // remaining (remaining == baseValue == 2), the fixed point the round-up
            // guard exists for - "2 + 2/2" = 3 in 2 operators (+, /).
            { 2, 3, 2 },
    ...
    public void LeastOpsExpressTargetByUnmemoizedRecursion_LeetCodeExamples_ReturnsFewestOperators(
        int baseValue, int target, int expected)
    {
        var actual = LeastOperatorsToExpressNumberSolution.LeastOpsExpressTargetByUnmemoizedRecursion(baseValue, target);
    ...
    public void LeastOpsExpressTargetByMemoizedRecursion_LeetCodeExamples_ReturnsFewestOperators(
        int baseValue, int target, int expected)
    {
        var actual = LeastOperatorsToExpressNumberSolution.LeastOpsExpressTargetByMemoizedRecursion(baseValue, target);

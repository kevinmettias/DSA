...
// are asserted against the same examples. sequenceLength = 1 and sequenceLength = 2
...
public sealed partial class NumberOfDistinctRollSequencesTests
{
    ...
    public void DistinctSequencesByBruteForceRecursion_LeetCodeExamples_ReturnsSequenceCount(
        int sequenceLength, long expected) =>
        Assert.Equal(expected, NumberOfDistinctRollSequencesSolution.DistinctSequencesByBruteForceRecursion(sequenceLength));

    [Theory]
    [MemberData(nameof(Examples))]
    public void DistinctSequencesByMemoizedRecursion_LeetCodeExamples_ReturnsSequenceCount(
        int sequenceLength, long expected) =>
        Assert.Equal(expected, NumberOfDistinctRollSequencesSolution.DistinctSequencesByMemoizedRecursion(sequenceLength));
}

...
public sealed class QueryKthSmallestTrimmedNumberTests
{
    ...
    {
        Assert.Equal(expected, QueryKthSmallestTrimmedNumberSolution.AnswerQueriesBySelectionScan(nums, queries));
    }
    ...
    public void AnswerQueriesByMergeSort_Example_ReturnsOriginalIndexOfKthSmallestTrimmed(
        string[] nums, int[][] queries, int[] expected)
    {
        Assert.Equal(expected, QueryKthSmallestTrimmedNumberSolution.AnswerQueriesByMergeSort(nums, queries));
    }
}

...
public sealed partial class MaximumFrequencyStackTests
{
    ...
    public void CreateByHashMapAndStack_LeetCodeExamples_PopsMostFrequentThenMostRecent(
        ...
        Assert.Equal(expected, RunScript(MaximumFrequencyStackSolution.CreateByHashMapAndStack(), operations));

    [Theory]
    [MemberData(nameof(Examples))]
    public void CreateByListRescan_LeetCodeExamples_PopsMostFrequentThenMostRecent(
        FreqStackOp[] operations, int?[] expected) =>
        Assert.Equal(expected, RunScript(MaximumFrequencyStackSolution.CreateByListRescan(), operations));

    private static int?[] RunScript(
        MaximumFrequencyStackSolution.IFreqStack freqStack, FreqStackOp[] operations) =>
        [.. operations.Select(operation => operation.Apply(freqStack))];

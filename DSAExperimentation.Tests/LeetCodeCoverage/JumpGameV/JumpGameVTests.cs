...
public sealed partial class JumpGameVTests
{
    ...
    {
        var actual = MaxIndicesVisited(arr, d);
    ...
    private static int MaxIndicesVisited(int[] arr, int d)
    {
        ...
        var ordering = TopologicalOrder(nodes);
        var longestPath = ComputeLongestPaths(ordering);

        return longestPath.Values.Max();
    }

    private static List<JumpNode> TopologicalOrder(JumpNode[] nodes)
    {
    ...
    private static Dictionary<JumpNode, int> ComputeLongestPaths(List<JumpNode> ordering)
    {
